...
void testRandomPlp(int probSize);
void testFastPlp(int probSize, int greedyMethod, int greedySubmethod, double paramA, int paramB, int hcParam);
...
int main(int argc, char **argv) {
	if(argc==2) {
		...
		testFastPlp(probSize, greedyMethod, greedySubmethod, paramA, paramB, hcParam);
	}
	return 0;
}
...
void testRandomPlp(int probSize) {
	...
	if(probSize == 0)
		...
		p = path("problem_instance/d" + to_string(probSize));
	for (recursive_directory_iterator i = recursive_directory_iterator(p); i != recursive_directory_iterator(); i++)
	{
		if (!is_directory(i->path())) //we eliminate directories
		{
			...
			ConflictGraph* conflictGraph = InstanceLoader::load(i->path().string());
			RandomPLP randomPlp(probSize, conflictGraph->getNumPositions());
			Solution* solution = randomPlp.produceSolution(conflictGraph);
			cout<<probSize<<"\t"<<instanceNo<<"\t"<<solution->getConflictSize()<<endl;
...
void testFastPlp(int probSize, int greedyMethod, int greedySubmethod, double paramA, int paramB, int hcParam)
{
	cout<<"Prob Size: "<<probSize<<" Greedy Method: "<<greedyMethod<<" Greedy Submethod: "<<greedySubmethod<<" Param A: "<<paramA<<" Param B: "<<paramB<<endl;
	...
	for (recursive_directory_iterator i = recursive_directory_iterator(p); i != recursive_directory_iterator(); i++)
	{
		if (!is_directory(i->path())) //we eliminate directories
		{
			...
			FastHeuristicPLPParameters fastHeuristicPLPParameters(probSize, 4, greedyMethod, greedySubmethod, paramA, paramB, hcParam, true);
			fastHeuristicPLP.init(fastHeuristicPLPParameters);
			Solution* solution = fastHeuristicPLP.optimize(conflictGraph1);
			...
			performanceResult[2]=fastHeuristicPLP.getResults().greedyConflictSize;
			performanceResult[3]=fastHeuristicPLP.getResults().greedyTime;
			performanceResult[4]=fastHeuristicPLP.getResults().localSearchConflictSize;
			performanceResult[5]=fastHeuristicPLP.getResults().greedyTime+fastHeuristicPLP.getResults().localSearchTime;
